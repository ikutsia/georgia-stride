...
const Header = ({ cartCount, onCartClick }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-br from-indigo-500 to-purple-600 text-white py-4 shadow-lg">
      <div className="max-w-6xl mx-auto px-8 flex justify-between items-center">
        ...
        <nav className={`hidden md:flex gap-8 ${isMenuOpen ? "md:flex" : ""}`}>
          <a
            href="#home"
            className="text-white no-underline font-medium transition-all duration-300 hover:text-yellow-400 hover:-translate-y-0.5 relative group"
          >
            Home
      ...
      {/* Mobile Navigation */}
      <nav
        className={`md:hidden fixed top-full left-0 right-0 bg-gradient-to-br from-indigo-500 to-purple-600 flex-col p-8 transform transition-all duration-300 ${
          isMenuOpen
      ...
      >
        <a
          href="#home"
          className="text-white no-underline font-medium py-2 block hover:text-yellow-400 transition-all duration-300"
        >
          Home
        </a>
        <a
